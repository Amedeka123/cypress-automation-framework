...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("filter() to retrieve DOM elements that match a specific selector", () => {
    cy.get("[data-toggle='buttons'] > *")
      .filter(".active")
      .should("contain", "Button-1");
  });

  it("find() to retrieve DOM elements of a given selector", () => {
    cy.get(".pagination").find(".page-link").should("have.length", 7)
  });
  ...
  it("siblings() To get all sibling DOM elements of elements", () => {
    cy.get(".traversal-button-other-states").find(".active").siblings().should("have.length",3)
  });
});
